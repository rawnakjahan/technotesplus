                            ...
                            switch (k) {
                                ...
                                case 'docexpired':
                                    if (v == null) {
                                        ...
                                        modal.find('input#docexpire').val(v);
                                ...
                                case 'DocumentType':
                                    ...
                                    if (SHOW_SELECTPICKER_DOC_TYPE_DROPDOWN === "True") {
                                        ...
                                        select.html(`<option selected='selected'
                                                value="${saved_doctype_id}"
                                                title="${res.document_type_details.full_path}"
                                                >${res.document_type_details.name}</option>`);
                                    }
                                    break;
                                case 'box_number':
                                            ...
                                            $.each(saved_search_metafield, function (k, v) {
                                                ...
                                                switch (v.data_type) {
                                                    ...
                                                    case 3:
                                                        ...
                                                        if (v.multiselect) {
                                                            $('#'+v.title).selectpicker('refresh');
                                                            $('#'+v.title).selectpicker('val', v.value);
                                                        }
                                                        break;
                                                    case 4:
                                                ...
                                                if (CLIENT_NAME === 'NRBC') {
                                                    if (v.title === 'voucher_date') {
                                                        console.log(1);
                                                        setTimeout(function () {
                                                            let $voucher_date = $('#voucher_date');
                                                            $('input[name="voucher_date"]').data('DateTimePicker').destroy();
                                                            ...
                                                            $voucher_date.val(v.value);
                                                        }, 1000);
                                                    }
                                                    if (v.title === 'date') {
                                                        console.log(2);
                                                        setTimeout(function () {
                                                            let $save_search_date = $('#date');
                                                            $('input[name="date"]').data('DateTimePicker').destroy();
                                                            $save_search_date.daterangepicker({
                                                                "opens": "right",
                                                                autoUpdateInput: false,
                                                                locale: {
                                                                    "cancelLabel": "Clear",
                                                            ...
                                                            $save_search_date.on('apply.daterangepicker', function (ev, picker) {
                                                                let from = moment(picker.startDate, 'YYYY-MM-DD hh:mm A').format();
                                                                let to = moment(picker.endDate, 'YYYY-MM-DD hh:mm A').format();
                                                                // creation_date_from = from;
                                                                // creation_date_to = to;
                                                            ...
                                                            $save_search_date.on('cancel.daterangepicker', function (ev, picker) {
                                                                $(this).val('');
                                                            });
                                                            $('.input-mini').addClass("date_form");
                                                            $('.applyBtn').removeClass('btn-success').addClass('btn-info');
                                                            $('.cancelBtn').removeClass('btn-default').addClass('bgm-bluegray');
                                                            $save_search_date.val(v.value);
                                                        }, 1000);
                                                    }
                                                }
