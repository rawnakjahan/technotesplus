...
//    })
    if (CLIENT_NAME === 'NRBC') {
        var search_date_from, search_date_to, search_voucher_from, search_voucher_to;
    }

    //--------Search onclick functions------------
    function searchReset() {
        if (SHOW_SELECTPICKER_DOC_TYPE_DROPDOWN === "True") {
...
//          document.location.href = search_result_url
    ...
    setTimeout(function () {
        ...
        $doccreated.on('apply.daterangepicker', function (ev, picker) {
            $(this).val(moment(picker.startDate).format('YYYY-MM-DD') + ' to ' + moment(picker.endDate).format('YYYY-MM-DD'));
        });
        $doccreated.on('cancel.daterangepicker', function (ev, picker) {
            $(this).val('');
        });
        ...
        $docexpired.on('apply.daterangepicker', function (ev, picker) {
            $(this).val(moment(picker.startDate).format('YYYY-MM-DD') + ' to ' + moment(picker.endDate).format('YYYY-MM-DD'));
        });
        $docexpired.on('cancel.daterangepicker', function (ev, picker) {
            $(this).val('');
        });
        $('.input-mini').addClass("date_form");
        $('.applyBtn').removeClass('btn-success').addClass('btn-info');
    ...
    function genDoc() {
        if (SHOW_SELECTPICKER_DOC_TYPE_DROPDOWN === "True") {
            let start = 0;  // set document set in dropdown from 1st object
            let length = 300;  // call 100 objects per request

            // again call for next 100 objects
            function callMore(start) {
                set_dropdown(start, length);
            }

            // initial call to set 1st 100 objects
            callMore(start);

            function set_dropdown(start = 0, length = 10) {
                $.ajax({
                    url: `/api/v1/dms/categorization/documenttype/?draw=1&start=${start}&length=${length}`,
                    method: 'GET',
                    success: function (data) {
                        let html = '';
                        $.each(data.data, function (k, v) {
                            let $this = v;
                            if (DMS_DOCTYPE_DISPLAY === "True") {
                                optionName = $this.name
                            } else {
                                optionName = $this.full_path;
                            }

                            var span_style = "display:block;"

                            html += `<option data-content="<span style='${span_style}' title='${$this.full_path}'> ${optionName}</span>" value="${$this.id}" ></option>`;
                        });
                        $('#documentType').append(html).selectpicker('refresh');

                        // set which next objects will call next
                        start = start + length;
                        if (start < data.recordsTotal) callMore(start);
            ...
            $("#documentType").select2({
                dropdownParent: $('#dms_search_form'),
                ajax: {
    ...
    $documentType.on('change', function () {
        ...
        if (CLIENT_NAME === 'NRBC') {
            console.log('NRBC');
            setTimeout(function () {
                $('#voucher_date').closest('.fg-line').empty().append(`Voucher Date : <input id="voucher_date" name="voucher_date" class="form-control" autocomplete="off">`);
                ...
                $('#voucher_date').on('apply.daterangepicker', function (ev, picker) {
                ...
                $('#voucher_date').on('cancel.daterangepicker', function (ev, picker) {
                    $(this).val('');
                });
                $('.input-mini').addClass("date_form");
                $('.applyBtn').removeClass('btn-success').addClass('btn-info');
            ...
            setTimeout(function () {
                $('#date').closest('.fg-line').empty().append(`Date : <input id="date" name="date" class="form-control" autocomplete="off">`);
    ...
    $('#search_btn').on('click', (e) => {
        ...
        if (CLIENT_NAME === "NRBC") {
            let voucher_date_range = {};
            voucher_date_range.from = search_voucher_from;
            voucher_date_range.to = search_voucher_to;
        }
        keyword.metas = {};
        ...
        data.user_id = userId;
        let $all_meta_data_fields = $('.meta-data-wrap input,.meta-data-wrap textarea,.meta-data-wrap select');
        ...
        if ($all_meta_data_fields.length > 0) {
            $all_meta_data_fields.each(function () {
                $all_meta_data_fields.serializeArray().map(function (x) {
                    if (keyword.metas.hasOwnProperty(x.name)) {
                        keyword.metas[x.name] = ($('#' + x.name).val()).toString();
                    } else {
                        keyword.metas[x.name] = x.value;
                    }
                })
        ...
        $.ajax({
            ...
            error: (res) => {
                if (res.status == 502) {
                    notify(res.responseJSON.message, '', '', 'danger', '5000');
                }
            }
        });
    });
    ...
    //------------- Search based on product DMS/Workflow ------------
    if (DMS != '' && WORKFLOW != '') {
        //
    } else if (DMS != '') {
        $('#search_text').on('keypress', function (e) {
            if (e.which == 13) {     // on press enter button
                $dms_normal_search.trigger('click');
            }
        });
    } else if (WORKFLOW != '') {
        $('#search_text').on('keypress', function (e) {
            if (e.which == 13) {     // on press enter button
                $workflow_normal_search.trigger('click');
            }
        });
    } else {
        //
...
if ($need_pass_change) {
    if (window.location.pathname !== '/admin/user/change-password') {
        window.location.href = '/admin/user/change-password'
    }
}
